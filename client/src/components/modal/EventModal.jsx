...
const EventModal = ({ show, onHideModal, event }) => {
  ...
  let eventTime = "";
  if(event) {
   eventTime = `${dayNames[event.start.getDay()]}, ${monthNames[event.start.getMonth()]} ${event.start.getDate()} • ${event.start.getHours()}:${event.start.getMinutes()} - ${event.end.getHours()}:${event.end.getMinutes()}`
  }
      ...
      {event && (
        <>
          <div className="flex flex-col gap-2 mb-8">
            <h2 className="text-2xl font-bold text-dark-purple">{event.title}</h2>
            <p>{eventTime}</p>
          </div>
